...
const Checkout: React.FC<CheckoutProps> = ({ pack, onBack }) => {
  ...
  const getTotalPrice = (): number => {
    ...
    if (pack.type === 'steam_topup') {
      ...
      return Math.ceil(baseRubWithMarkup * (1 + comm));
    }
  ...
  const handlePayment = async () => {
    ...
    // БЕЗОПАСНЫЙ ДОСТУП К ДАННЫМ ТЕЛЕГРАМА (Фикс ошибки null reading id)
    const telegramUser = tg?.initDataUnsafe?.user || null;
    const user_chat_id = telegramUser?.id || 0; 
    const firstName = telegramUser?.first_name || 'Web';
  ...
  return (
    <div className="flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-12 px-4 max-w-md mx-auto relative z-10 text-white">
      ...
      <div className="flex items-center gap-4 pt-6">
        <button onClick={() => { triggerHaptic('light'); onBack(); }} className="p-3 bg-white/10 backdrop-blur-md rounded-2xl border border-white/30">
          <ChevronLeft size={20} />
        </button>
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-2 gap-4">
          {[ { id: 'sbp' as const, img: '/sbp.jpg' }, { id: 'card' as const, img: '/card.jpg' } ].map((m) => (
            <button key={m.id} onClick={() => { triggerHaptic('light'); setPaymentMethod(m.id); }} className={`h-20 rounded-[24px] border-2 transition-all flex items-center justify-center relative ${paymentMethod === m.id ? 'bg-amber-500/10 border-amber-500 shadow-[0_0_20px_rgba(245,158,11,0.2)]' : 'bg-white/5 border-white/5 opacity-60'}`}>
      ...
      <div className="mt-auto space-y-4 pt-4">
        ...
        <button onClick={() => { triggerHaptic('heavy'); handlePayment(); }} disabled={isLoading || (!pack.is_code && !uid.trim())} className="w-full bg-amber-500 hover:bg-amber-400 py-6 rounded-[24px] font-black text-black text-xl uppercase transition-all active:scale-[0.97] disabled:opacity-50 shadow-xl shadow-amber-900/20">
