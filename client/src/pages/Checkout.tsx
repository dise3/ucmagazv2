...
const Checkout: React.FC<CheckoutProps> = ({ pack, onBack }) => {
  ...
  const getTotalPrice = (): number => {
    ...
    if (pack.type === 'steam_topup') {
      ...
      // Добавляем + 1 для синхронизации со страницей Steam.tsx
  ...
  const handlePayment = async () => {
    ...
    // МАКСИМАЛЬНО БЕЗОПАСНОЕ ПОЛУЧЕНИЕ ID (Исправление ошибки null reading 'id')
    let user_chat_id = 0;
    let buyer_first_name = 'Web';
    let buyer_last_name = 'User';

    if (tg && tg.initDataUnsafe && tg.initDataUnsafe.user) {
        user_chat_id = tg.initDataUnsafe.user.id;
        buyer_first_name = tg.initDataUnsafe.user.first_name || 'Web';
        buyer_last_name = tg.initDataUnsafe.user.last_name || 'User';
    }
