  ...
  return (
    <div className="fixed inset-0 bg-black/90 backdrop-blur-xl z-[200] flex flex-col items-center justify-center px-6 text-center animate-in fade-in duration-500">
      <div className="w-full max-w-xs space-y-8">
        ...
        <button onClick={onClose} className="w-full bg-white/10 hover:bg-white/20 py-5 rounded-2xl text-white font-black uppercase flex items-center justify-center gap-3 border border-white/10">
...
interface CheckoutProps {
  pack: { 
    ...
    months?: number;
    uid?: string;
    items?: Array<{ id: number; amount: number; price: number; quantity: number }>; // Фикс ошибки TS
  };
  onBack: () => void;
}
...
const Checkout: React.FC<CheckoutProps> = ({ pack, onBack }) => {
  ...
  const getTotalPrice = (): number => {
    ...
    // 1. Логика Steam (соответствует странице ввода)
    if (pack.type === 'steam_topup') {
      ...
      // В режиме receive на странице Steam была формула: base * Markup * Commision + 1
    ...
    // 3. UC (Промокоды или пакеты)
    const comm = paymentMethod === 'sbp' ? COMMISSION_SBP : COMMISSION_CARD;
    return Math.ceil((pack.price || 0) * (1 + (settings?.fee_percent || 0)) / (1 + COMMISSION_SBP) * (1 + comm));
  };
  ...
  const handlePayment = async () => {
    ...
    // ФИКС ОШИБКИ: Безопасное получение ID (если null -> 0)
    const telegramUser = tg?.initDataUnsafe?.user || null;
    const user_chat_id = telegramUser ? telegramUser.id : 0; 
    ...
    try {
      const response = await fetch(`${VITE_API_NGROK}/api/create-payment`, {
        ...
        body: JSON.stringify({
          ...
          buyer_first_name: telegramUser?.first_name || 'Web',
          buyer_last_name: telegramUser?.last_name || 'User',
          type: pack.type || 'uc', // Отправляем steam_topup если он выбран
    ...
    <div className="flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-12 px-4 max-w-md mx-auto relative z-10 text-white">
      ...
      <div className={`fixed bottom-0 left-0 right-0 z-[101] bg-[#1c1c21] border-t border-white/10 rounded-t-[40px] transition-transform duration-500 ${showHelp ? 'translate-y-0' : 'translate-y-full'}`} style={{ height: '70%' }}>
        <div className="px-6 flex justify-between items-center mt-8 mb-6">
          ...
          <button onClick={() => setShowHelp(false)} className="p-3 bg-white/5 rounded-full text-white/50 active:scale-90 transition-all"><X size={24} /></button>
        </div>
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-2 gap-4">
          {[ { id: 'sbp' as const, img: '/sbp.jpg' }, { id: 'card' as const, img: '/card.jpg' } ].map((m) => (
            <button key={m.id} onClick={() => { if(tg?.HapticFeedback) tg.HapticFeedback.impactOccurred('light'); setPaymentMethod(m.id); }} className={`h-20 rounded-[24px] border-2 transition-all flex items-center justify-center relative ${paymentMethod === m.id ? 'bg-amber-500/10 border-amber-500 shadow-[0_0_20px_rgba(245,158,11,0.2)]' : 'bg-white/5 border-white/5 opacity-60'}`}>
              <img src={m.img} className="h-8 object-contain" alt={m.id} />
              {paymentMethod === m.id && <div className="absolute -top-2 -right-2 bg-amber-500 rounded-full p-1"><CheckCircle2 size={14} className="text-black" strokeWidth={4} /></div>}
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="mt-auto space-y-4 pt-4">
        ...
        <button onClick={() => { if(tg?.HapticFeedback) tg.HapticFeedback.impactOccurred('heavy'); handlePayment(); }} disabled={isLoading || (!pack.is_code && !uid.trim())} className="w-full bg-amber-500 hover:bg-amber-400 py-6 rounded-[24px] font-black text-black text-xl uppercase transition-all active:scale-[0.97] disabled:opacity-50 shadow-xl shadow-amber-900/20">
